...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (!documentTypes.length) {
        ...
        documentTypes = [
            { type: 'PDF Documents', count: Math.floor(totalDocs * 0.45) },
            { type: 'Word Documents', count: Math.floor(totalDocs * 0.30) },
            { type: 'Excel Files', count: Math.floor(totalDocs * 0.15) },
            { type: 'PowerPoint', count: Math.floor(totalDocs * 0.10) }
        ];
    }
    ...
    if (!categoryBreakdown.length) {
        categoryBreakdown = [
            { category: 'Legal Documents', count: Math.floor(totalCategories * 0.25) },
            { category: 'Financial Records', count: Math.floor(totalCategories * 0.20) },
            { category: 'HR Documents', count: Math.floor(totalCategories * 0.18) },
            { category: 'Operations', count: Math.floor(totalCategories * 0.15) },
            { category: 'Marketing', count: Math.floor(totalCategories * 0.12) },
            { category: 'Others', count: Math.floor(totalCategories * 0.10) }
        ];
    }
    ...
    document.getElementById('darkModeToggle').addEventListener('click', function() {
        ...
        [barChart, lineChart, pieChart, doughnutChart, validationStatusChart, validationTrendChart].forEach(chart => {
